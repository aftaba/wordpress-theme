...
$tickets = Tribe__Tickets_Plus__Commerce__EDD__Main::get_instance()->get_all_event_tickets( get_the_ID() );
if ( empty( $tickets ) ) {
	$total_stock = 0;
} else {
	$event_ticket_present = 1;
	$total_stock = $tickets[0]->stock();
}


?>
<?php if ( ( $total_stock ) && ( 'no' == $private_event ) ) : ?>
...
<?php elseif ( $total_stock ) : ?>
	<div class="<?php tribe_events_event_classes() ?>" data-tribejson='<?php echo esc_attr( tribe_events_template_data( $post, $additional_data ) ); ?>'>
		<h3 class="tribe-events-month-event-title private-event-title-only"><?php echo $title ?></h3>
	</div><!-- #tribe-events-event-# -->
<?php elseif ( 1 == $event_ticket_present ) : ?>
	<div class="<?php tribe_events_event_classes() ?>" data-tribejson='<?php echo esc_attr( tribe_events_template_data( $post, $additional_data ) ); ?>'>
		<h3 class="tribe-events-month-event-title sold-event-title-only"><?php echo $title;?></h3>
	</div><!-- #tribe-events-event-# -->
<?php endif; ?>
